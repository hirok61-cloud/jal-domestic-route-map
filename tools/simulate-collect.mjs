...
const cases = [
  ...
  {
    name: "座席表が1件も返ってこないと、運賃は保存したうえで失敗として気づける",
    /* CONTENT_VERSIONがずれるとJALのseatmap APIがHTTP 200のままエラー本文を
       返すようになり、これと見分けがつかない状態になる。2026-08-15に実際に
       48時間気づかず座席詳細が更新され続けなかった。個々の失敗（タイムアウト等）
       で打ち切らないのは変えないが、全便ぶん試して1件も取れなければ
       保存はしたうえで理由を出すようにした。 */
    jal: (seat) => (seat ? new Promise(() => {}) : jalOk(false)), // 座席表だけ永久に待たせる
    saver: () => res(200, { ok: true }),
    check: (log, out) => ({
      // 1日目の70便を打ち切らず試したところで失敗と判定するので、2日目には進まない
      "1日目は打ち切らず70便ぶん試している": log.seatmaps === 70,
      "運賃の保存は行われる（区切り保存含む・計3回）": log.saved.length === 3,
      "1件も取れていないので失敗として報告する": !ok(out),
      "CONTENT_VERSIONを疑う理由を出す": /CONTENT_VERSION/.test(text(out)),
      "運賃までは保存済みだと分かる": /保存済み/.test(text(out)),
    }),
  },
  {
    name: "座席表の一部だけ返ってこなくても完走する",
    /* こちらは「一部の便だけたまたま取得に失敗する」通常の揺らぎ。
       全便が失敗する上のケースとは分けて確かめる。 */
    jal: (() => {
      let n = 0;
      return (seat) => (seat && n++ % 3 === 0 ? new Promise(() => {}) : jalOk(seat));
    })(),
    saver: () => res(200, { ok: true }),
    check: (log, out) => ({
