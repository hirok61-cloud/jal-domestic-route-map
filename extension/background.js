...
/* 制度枠(JOH)の自動枠は置いていない。**JALオンラインにログイン済みのタブが
   必須**で、無操作セッションは約1時間で切れるため、深夜に自動で走らせても
   ほぼ失敗するだけになる（2026-08-15、本人判断で自動化は見送り）。
   制度枠はサイトのボタンかスマホのブックマークレットから手動で記録する。
   ハブ別の見送りガード（lastAutoBy）は、将来また足すときのために残してある。 */
const AUTO_SCHEDULE = [
