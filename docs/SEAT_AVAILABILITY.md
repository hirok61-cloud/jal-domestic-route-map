...
- `/seats/` — 区間ごとの便一覧（便名・時刻・機材・残席・最安運賃）。
  ...
  自動的に翌日を開く
- `/`（ルートマップ）— **今日ぶんのみ**。サブタブ横の「空席で塗り分け」ボタンで、地図の路線を
...
JALが当日空港割り当てぶんの座席を確保しているため、座席表側は埋まっていることがある。
...
- **夜間は全区間が `cancelled` で返る。** 最終便まで出発済みだとJALは
  ...
  *全区間が cancelled かつ当日* なら「本日の運航は終了」として表示する。
  ...
  セッション拒否とみなして保存を見送る。
- **ブックマークレットは収集ロジックを埋め込む。外部読み込みはしない。**
  JALのページから外部スクリプトを読む方式は iOS / Android の両方で失敗した。
  ただし素のソースを埋め込むと 34,695 字になり、**Chromeのアドレスバーの上限
  32,768 字で切り捨てられて何も起きなくなる**（実測）。日本語は1文字が `%XX` 表記で
  9字に膨らむため、コメントとUI文言だけで上限に届く。
  そこで **`seats/collect.min.js`（コメントを落とした圧縮版）を埋め込む**ことで
  約1万6千字に収めている。押した瞬間に小さな枠を出し、実行中のホスト名と
  失敗理由を表示するので「何も起こらない」状態にはならない。
- **収集ロジックを直したら、必ず作り直すこと。**
  ...
  `tools/collect-in-browser.js` → `seats/collect.min.js` を再生成し、
  ブックマークレットが上限に収まるかも確認する。生成物もコミットする。
  埋め込み方式なので、**利用者は登録し直しが必要**になる。
