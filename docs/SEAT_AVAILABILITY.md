...
- **夜間は全区間が `cancelled` で返る。** 最終便まで出発済みだとJALは
  ...
  *全区間が cancelled かつ当日* なら「本日の運航は終了」として表示する。
  ...
  セッション拒否とみなして保存を見送る。
- **ブックマークレットは「起動役」だけにして小さく保つ（722字）。**
  収集ロジックを丸ごと埋め込むと 34,695 字になり、**Chromeのアドレスバーの上限
  32,768 字で切り捨てられて何も起きなくなる**（実測）。日本語は1文字が `%XX` 表記で
  9字に膨らむため、コメントとUI文言だけですぐ上限に届く。
  代わりに、押した瞬間に必ず小さな枠を表示し、実行中のホスト名と失敗理由を出す。
  こうしておくと「何も起こらない」状態にならず、切り分けができる。
